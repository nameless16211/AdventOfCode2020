"use strict";
var AOC2 = /** @class */ (function () {
    function AOC2() {
    }
    /**
     * --- Day 2: Password Philosophy Part 1 ---
     */
    AOC2.validateOldPasswords = function (passwords) {
        var counter = 0;
        for (var i = 0; i < passwords.length; i++) {
            var splitresult = passwords[i].split(":");
            var expstring = splitresult[0].split(" ");
            var pass = splitresult[1].trim();
            var min = Number.parseInt(expstring[0].split("-")[0]);
            var max = Number.parseInt(expstring[0].split("-")[1]);
            var char = expstring[1].trim();
            var res = this.countLetters(min, max, char, pass);
            if (res)
                counter++;
        }
        return counter;
    };
    AOC2.countLetters = function (min, max, char, string) {
        var counter = 0;
    ...
    AOC2.validateNewPasswords = function (passwords) {
        var counter = 0;
        for (var i = 0; i < passwords.length; i++) {
            var splitresult = passwords[i].split(":");
            var expstring = splitresult[0].split(" ");
            var pass = splitresult[1].trim();
            var first = Number.parseInt(expstring[0].split("-")[0]);
            var second = Number.parseInt(expstring[0].split("-")[1]);
            var char = expstring[1].trim();
            var res = this.checkCharacters(first, second, char, pass);
            if (res)
                counter++;
        }
        console.log(counter);
    ...
    AOC2.checkCharacters = function (first, second, char, pass) {
        var flag = false;
