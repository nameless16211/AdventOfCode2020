class AOC2
{
     ...
     */
    static validateOldPasswords(passwords: string[])
    {
        let counter = 0;
        for(let i = 0; i < passwords.length; i++)
        {
            let splitresult = passwords[i].split(":");

            let expstring = splitresult[0].split(" ");
            let pass = splitresult[1].trim();

            let min = Number.parseInt(expstring[0].split("-")[0]);
            let max = Number.parseInt(expstring[0].split("-")[1]);
            let char = expstring[1].trim();

            let res = this.countLetters(min, max, char, pass);
    ...
    private static countLetters(min: number, max: number, char: string, string: string): boolean
    {
        let counter = 0;
    ...
    static validateNewPasswords(passwords: string[])
    {
        let counter = 0;
        for(let i = 0; i < passwords.length; i++)
        {
            ...
            let first = Number.parseInt(expstring[0].split("-")[0]);
            let second = Number.parseInt(expstring[0].split("-")[1]);
            let char = expstring[1].trim();

            let res = this.checkCharacters(first, second, char, pass);
    ...
    private static checkCharacters(first: number, second: number, char: string, pass: string): boolean
    {
        let flag = false;

        flag = pass[first - 1] === char;

        flag = pass[second - 1] === char ? ( flag ? false : true ) : flag;

        return flag;
    }

}
let arr = ["9-10 m: mmmmnxmmmwm",
"6-8 w: wpwwhxnv",
"4-6 n: trwpnnnvq",
...
AOC2.validateOldPasswords(arr);

AOC2.validateNewPasswords(arr);